...
window.winPlayer=(p)=>{ skinsGame.winPlayer(p,players,ledger,baseWager); finishHole(); };
window.winTeam=(t)=>{ skinsGame.winTeam(t,teams,ledger,baseWager); finishHole(); };
window.tieHole=()=>{ skinsGame.tie(); finishHole(); };
...
players.forEach(p=>leaderboard.innerHTML+=`${p}: $${ledger[p]}<br>`);
}
